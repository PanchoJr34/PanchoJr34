<h1 align="center">¡Hola! Soy Francisco Jimenez 👋</h1>

<p align="center">
  <b>Ingeniero en Tecnologías de la Información</b><br>
  Especialista en Inteligencia Artificial y Automatización de Procesos | Desarrollador Web Jr.
</p>

<p align="center">
  📍 <b>Ubicación:</b> Jiutepec, Morelos, México &nbsp;|&nbsp;
  🏢 <b>Trabajo actual:</b> Grupo AMDOSA S.A. de C.V. &nbsp;|&nbsp;
  🎓 <b>Formación:</b> UPEMOR &nbsp;|&nbsp;
  🚀 <b>Enfoque:</b> IA, Automatización y Transformación Digital
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/francisco-j-214254355">
    <img src="https://img.shields.io/badge/LinkedIn-Francisco%20Jimenez-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:soyfrancisco.js31@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-soyfrancisco.js31%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

---

## 👨‍💻 Acerca de mí

Soy **Ingeniero en Tecnologías de la Información** egresado de la **Universidad Politécnica del Estado de Morelos (UPEMOR)**. Tengo experiencia en **desarrollo web Full-Stack**, soporte de sistemas, documentación técnica, bases de datos, control de versiones y automatización de procesos.

Actualmente trabajo en **Grupo AMDOSA S.A. de C.V.**, enfocado en **Inteligencia Artificial, automatización de procesos, transformación digital, análisis de flujos de trabajo y mejora operativa**. Me interesa crear soluciones tecnológicas prácticas que ayuden a las empresas a reducir tiempos, mejorar procesos y tomar mejores decisiones.

---

## 🎯 Enfoque actual

- 🤖 Automatización de procesos mediante **Inteligencia Artificial**.
- 🔄 Mapeo, análisis y mejora de flujos de trabajo empresariales.
- 📊 Documentación ejecutiva, reportes y análisis de información.
- ⚙️ Optimización de tareas administrativas y operativas.
- 🏭 Transformación digital aplicada a entornos industriales y empresariales.
- 🧩 Integración de herramientas digitales para mejorar productividad, control y seguimiento.

---

## 🧰 Tecnologías y herramientas

### 💻 Lenguajes

<p>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-323330?style=for-the-badge&logo=javascript&logoColor=F7DF1E" />
  <img src="https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-025E8C?style=for-the-badge&logo=database&logoColor=white" />
  <img src="https://img.shields.io/badge/C%23-68217A?style=for-the-badge&logo=csharp&logoColor=white" />
</p>

### 🚀 Frameworks y librerías

<p>
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/Laravel-FF2D20?style=for-the-badge&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/Node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white" />
  <img src="https://img.shields.io/badge/jQuery-0769AD?style=for-the-badge&logo=jquery&logoColor=white" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" />
</p>

### 🗄️ Bases de datos, control de versiones y herramientas

<p>
  <img src="https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/DBeaver-382923?style=for-the-badge&logo=dbeaver&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-F05033?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-0DB7ED?style=for-the-badge&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker_Compose-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
</p>

### 🤖 IA, productividad y metodología

<p>
  <img src="https://img.shields.io/badge/Inteligencia_Artificial-412991?style=for-the-badge&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/Automatización-FF6F00?style=for-the-badge&logo=zapier&logoColor=white" />
  <img src="https://img.shields.io/badge/Mapeo_de_Procesos-0052CC?style=for-the-badge&logo=diagramsdotnet&logoColor=white" />
  <img src="https://img.shields.io/badge/Scrum-6DB33F?style=for-the-badge&logo=trello&logoColor=white" />
  <img src="https://img.shields.io/badge/MVC-000000?style=for-the-badge&logo=codeigniter&logoColor=white" />
  <img src="https://img.shields.io/badge/Excel_Avanzado-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white" />
  <img src="https://img.shields.io/badge/SAP_Básico-0FAAFF?style=for-the-badge&logo=sap&logoColor=white" />
</p>

---

## 💼 Experiencia profesional

### 🏢 Grupo AMDOSA S.A. de C.V.
**Especialista en Inteligencia Artificial y Transformación Digital**  
**Actualidad**

- Análisis de procesos internos para detectar oportunidades de automatización.
- Implementación de herramientas de IA para productividad, documentación y mejora operativa.
- Elaboración de reportes ejecutivos, propuestas digitales y documentación de procesos.
- Mapeo de flujos de trabajo para mejorar control, seguimiento y eficiencia.
- Participación en iniciativas de transformación digital aplicadas a áreas administrativas, comerciales e industriales.

---

### 🏛️ H. Ayuntamiento de Jiutepec — Secretaría Municipal
**Auxiliar Estratégico y Encargado de Sistemas**  
**2025 - 2026**

- Asistencia directa al Secretario Municipal en la coordinación de asuntos prioritarios, control de documentación oficial y enlace entre diversas dependencias del ayuntamiento.
- Gestión del área de sistemas, asegurando el óptimo funcionamiento de plataformas internas, bases de datos y automatización de tareas administrativas para optimizar los flujos de trabajo operativos.

---

### 🏭 Nissan Mexicana — Planta CIVAC
**Técnico de Mantenimiento de fines de semana**  
**Julio 2025 - Enero 2026**

- Ejecución de mantenimiento preventivo y correctivo en las líneas de ensamble de modelos Frontier y NP300, asegurando el cumplimiento de estrictos estándares operativos.
- Registro formal de actividades operativas mediante el uso de bitácoras, checklists y seguimiento de incidencias.

---

### 🔧 Taller de Reparación Especializado
**Encargado de Taller**  
**2023 - 2024**

- Diagnóstico y reparación de componentes electrónicos complejos.
- Atención directa al público para resolución de requerimientos técnicos.
- Organización de actividades del taller y seguimiento de equipos en reparación.

---

## 🚀 Proyectos tecnológicos

### 🛡️ SICAE — Sistema Web contra el Acoso Escolar
**Desarrollador Full-Stack**  
**Septiembre 2025 - Abril 2026**

Plataforma web para denunciar y gestionar casos de bullying y ciberacoso de forma segura y anónima en instituciones educativas.

**Tecnologías:** React, Laravel, Node.js, Tailwind CSS, MySQL, Docker, Git, MVC.

- Desarrollo de interfaces web utilizando React.js, HTML5 y CSS3.
- Implementación de funcionalidades backend mediante PHP, Laravel y Node.js.
- Gestión de bases de datos relacionales y control de versiones con Git.
- Enfoque en confidencialidad, seguimiento administrativo y comunicación segura.

---

### 🏨 HotelHub — Plataforma de Gestión Hotelera
**Desarrollador Web**  
**Agosto 2024 - Diciembre 2024**

Sistema de administración hotelera para reservaciones, inventario y reportes operativos.

**Tecnologías:** PHP nativo, JavaScript, Tailwind CSS, MySQL, MVC.

- Desarrollo de módulos de reservaciones, gestión de inventario y generación automática de reportes.
- Implementación y diseño de base de datos utilizando arquitectura MVC con PHP puro.

---

### 🎮 Videojuego Educativo 2D
**Desarrollador Lógico — Unity**  
**Septiembre 2022 - Diciembre 2022**

Proyecto educativo desarrollado en Unity con lógica de juego e interacción básica.

**Tecnologías:** Unity, C#.

- Programación en C# de físicas, control de colisiones e inteligencia artificial básica para comportamiento de enemigos.

---

## 🎓 Educación

| Nivel | Institución | Carrera / Especialidad | Periodo |
|---|---|---|---|
| 🎓 Ingeniería | Universidad Politécnica del Estado de Morelos — UPEMOR | Ingeniería en Tecnologías de la Información | 2021 - Abril 2026 |
| 💻 Técnico | CETIS No. 44 | Técnico en Programación | 2018 - 2021 |

---

## 🏆 Logros

- 🥇 **1er Lugar Internacional** — Rally Latinoamericano de Innovación ANFEI 2024, categoría Impacto Social.
- 🥇 **1er Lugar Nacional** — Rally Latinoamericano de Innovación ANFEI 2024.
- 💡 Desarrollo en equipo de una solución tecnológica orientada a reducir el desperdicio de alimentos.

---

## 🌎 Idiomas

| Idioma | Nivel |
|---|---|
| 🇪🇸 Español | Nativo |
| 🇺🇸 Inglés | Avanzado C1 |
| 🇫🇷 Francés | Básico A2 |

---

## 📌 Resumen profesional

```txt
Ingeniero en Tecnologías de la Información con experiencia en desarrollo web Full-Stack,
soporte de sistemas, documentación técnica, bases de datos y automatización de procesos.

Actualmente enfocado en Inteligencia Artificial, transformación digital, análisis de procesos
y mejora operativa dentro de entornos empresariales e industriales.
```

---

<p align="center">
  <i>“La tecnología tiene más valor cuando resuelve problemas reales, mejora procesos y ayuda a las personas a trabajar mejor.”</i>
</p>
